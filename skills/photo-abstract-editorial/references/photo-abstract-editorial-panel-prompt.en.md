# Photo Abstract Editorial — Abstract Panel (English)

Treat the uploaded image strictly as the sole source of content. Create one **abstract memory panel with a poetic English title** derived entirely from the photograph. Do not simply apply a filter to the photo.

## 1. Role of the input image

The uploaded image is the sole content source. Read it only as an **information source for the abstract panel.** Analyze the photo's subject relationships; relative scale; left/right and upper/lower positions; directions; curves; horizontal and vertical axes; spacing; repetition; occlusion; foreground/background hierarchy; tonal hierarchy; color roles; and negative space. Reconstruct these relationships as an abstract visual memory on the panel.
...
The entire output canvas is the ivory abstract panel. Match the source photograph's aspect ratio. Do not force 1:1, 3:4, or other ratios unrelated to the source image.

Do not use torn-paper edges, frames, drop shadows, dimensional cards, tape, collage shadows, or mockup effects.
...
Never place the title inside the motif, at the lower right, or on the canvas edge.
...
Output only one finished abstract memory panel filling the entire canvas.
