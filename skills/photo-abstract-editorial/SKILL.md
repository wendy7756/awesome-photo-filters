...
- Use Codex built-in **image generation / editing** (`image_gen`). No OpenRouter or external API key is required.
- Read the full prompt file(s) below before generating.
- Preserve the uploaded photo's subjects, composition, and identity in the photographic area.
- Default to the **full composite** workflow unless the user asks for the abstract panel only.

## Outputs

| Output | Prompt file | When to use |
| --- | --- | --- |
| **Full composite** (photo + abstract panel + title) | [references/photo-abstract-editorial-prompt.en.md](references/photo-abstract-editorial-prompt.en.md) | Default finished artwork |
| **Abstract panel only** | [references/photo-abstract-editorial-panel-prompt.en.md](references/photo-abstract-editorial-panel-prompt.en.md) | User wants only the lower panel / result preview |

When producing the full composite, adapt photo/panel height proportions to the source image orientation (landscape, portrait, or square) rather than splitting the canvas mechanically in half. See [layout.ts](layout.ts) for ratio guidance.
...
- **Composite (default):** [references/photo-abstract-editorial-prompt.en.md](references/photo-abstract-editorial-prompt.en.md)
- **Panel only:** [references/photo-abstract-editorial-panel-prompt.en.md](references/photo-abstract-editorial-panel-prompt.en.md)

## Upstream

Based on [photo-abstract-editorial](https://github.com/ZzzLc0405/photo-abstract-editorial), integrated in [awesome-photo-filters](https://github.com/wendy7756/awesome-photo-filters).
